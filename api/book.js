...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const conflictData = await creatorGet(
      `report/All_Spaces?criteria=${encodeURIComponent(`Inventory_Items == ${inventoryItemId} && Booking_Start <= "${toCreatorDate(booking_end)}" && Booking_End >= "${toCreatorDate(booking_start)}"`)}&limit=1`,
      token
    )
